...
include("include/tvshowview_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["tvshowview_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "tvshowview_search.htm";
...
$params['shortTableName'] = 'tvshowview';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("idParentPath"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("idParentPath")] = GetTableURL($pageObject->pSet->getLookupTable("idParentPath"));
	
	$pageObject->fillFieldToolTips("idParentPath");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("idParentPath");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "idParentPath";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("idParentPath_label","<label for=\"".GetInputElementId("idParentPath", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("idParentPath_label", true);
	
	$xt->assign("idParentPath_fieldblock", true);
	$xt->assignbyref("idParentPath_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("idParentPath_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("idParentPath_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_idParentPath", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("idParentPath");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idParentPath", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idParentPath", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("totalCount"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("totalCount")] = GetTableURL($pageObject->pSet->getLookupTable("totalCount"));
	
	$pageObject->fillFieldToolTips("totalCount");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("totalCount");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "totalCount";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("totalCount_label","<label for=\"".GetInputElementId("totalCount", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("totalCount_label", true);
	
	$xt->assign("totalCount_fieldblock", true);
	$xt->assignbyref("totalCount_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("totalCount_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("totalCount_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_totalCount", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("totalCount");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"totalCount", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"totalCount", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("watchedcount"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("watchedcount")] = GetTableURL($pageObject->pSet->getLookupTable("watchedcount"));
	
	$pageObject->fillFieldToolTips("watchedcount");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("watchedcount");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "watchedcount";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("watchedcount_label","<label for=\"".GetInputElementId("watchedcount", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("watchedcount_label", true);
	
	$xt->assign("watchedcount_fieldblock", true);
	$xt->assignbyref("watchedcount_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("watchedcount_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("watchedcount_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_watchedcount", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("watchedcount");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"watchedcount", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"watchedcount", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("totalSeasons"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("totalSeasons")] = GetTableURL($pageObject->pSet->getLookupTable("totalSeasons"));
	
	$pageObject->fillFieldToolTips("totalSeasons");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("totalSeasons");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "totalSeasons";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("totalSeasons_label","<label for=\"".GetInputElementId("totalSeasons", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("totalSeasons_label", true);
	
	$xt->assign("totalSeasons_fieldblock", true);
	$xt->assignbyref("totalSeasons_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("totalSeasons_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("totalSeasons_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_totalSeasons", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("totalSeasons");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"totalSeasons", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"totalSeasons", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
