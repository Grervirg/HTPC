...
$layout->blocks["top"][] = "export";$page_layouts["tvshowview_export"] = $layout;
...
$xt->display("tvshowview_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=tvshowview.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=tvshowview.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=tvshowview.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["idParentPath"] = $pageObject->showDBValue("idParentPath", $row);
			...
			$values["totalCount"] = $pageObject->showDBValue("totalCount", $row);
			$values["watchedcount"] = $pageObject->showDBValue("watchedcount", $row);
			$values["totalSeasons"] = $pageObject->showDBValue("totalSeasons", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=tvshowview.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["idParentPath"] = $pageObject->getViewControl("idParentPath")->showDBValue($row, "");
			...
			$values["totalCount"] = $pageObject->getViewControl("totalCount")->showDBValue($row, "");
			$values["watchedcount"] = $pageObject->getViewControl("watchedcount")->showDBValue($row, "");
			$values["totalSeasons"] = $pageObject->getViewControl("totalSeasons")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["idParentPath"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["totalCount"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["watchedcount"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["totalSeasons"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Parent Path").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Watchedcount").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Seasons").'</td>';	
	}
	else
	{
		echo "<td>"."Id Show"."</td>";
		...
		echo "<td>"."C22"."</td>";
		echo "<td>"."C23"."</td>";
		echo "<td>"."Id Parent Path"."</td>";
		echo "<td>"."Str Path"."</td>";
		echo "<td>"."Date Added"."</td>";
		echo "<td>"."Last Played"."</td>";
		echo "<td>"."Total Count"."</td>";
		echo "<td>"."Watchedcount"."</td>";
		echo "<td>"."Total Seasons"."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									echo $values["watchedcount"];
			echo '</td>';
							echo '<td>';
			
									echo $values["totalSeasons"];
			echo '</td>';
			echo "</tr>";
		}
