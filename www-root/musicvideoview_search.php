...
$layout->blocks["top"][] = "search";$page_layouts["musicvideoview_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "musicvideoview_search.htm";
...
$params['shortTableName'] = 'musicvideoview';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("idMVideo"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("idMVideo")] = GetTableURL($pageObject->pSet->getLookupTable("idMVideo"));
	
	$pageObject->fillFieldToolTips("idMVideo");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("idMVideo");
	...
	{
		$firstFieldParams['fName'] = "idMVideo";
	...
	if(isEnableSection508())
		$xt->assign_section("idMVideo_label","<label for=\"".GetInputElementId("idMVideo", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("idMVideo_label", true);
	
	$xt->assign("idMVideo_fieldblock", true);
	$xt->assignbyref("idMVideo_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("idMVideo_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("idMVideo_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_idMVideo", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("idMVideo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idMVideo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"idMVideo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
