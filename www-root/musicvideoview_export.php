...
include("include/musicvideoview_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["musicvideoview_export"] = $layout;
...
$xt->display("musicvideoview_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=musicvideoview.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=musicvideoview.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=musicvideoview.xml");
	...
	{
		$values = array();
			$values["idMVideo"] = $pageObject->showDBValue("idMVideo", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=musicvideoview.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["idMVideo"] = $pageObject->getViewControl("idMVideo")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["idMVideo"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id MVideo").'</td>';	
